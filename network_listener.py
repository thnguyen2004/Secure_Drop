import socket
import json
import threading
from pathlib import Path

# NOTE: Using the standard 'input' here for user approval, which can interfere with the shell.
# The user must type 'y' or 'n' carefully when prompted in the recipient's terminal.
# from getpass import getpass # Using input() for thread safety simplicity

from network import port_for_email, id_for_email, BUFFER_SIZE
from contacts import get_my_contacts, upsert_contact_public_key
from crypto_utils import (
    verify_sig, sign_bytes, unwrap_aes_key, 
    decrypt_file, calculate_sha256, _b64d, _b64e
)

# --- Listener Setup ---

def start_listener(session: dict):
  t = threading.Thread(target=_listener_loop, args=(session,), daemon=True)
  t.start()

def _listener_loop(session: dict):
  port = port_for_email(session["email"])
  print(f"[LISTENER] Starting listener for {session['email']} on port {port}")

  sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
  sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
  
  try:
    sock.bind(("", port))
  except OSError as e:
    print(f"[ERROR] Could not bind to port {port}. Is another instance running? ({e})")
    return
    
  sock.listen(5)

  print(f"[LISTENER] Listening on port {port}")

  while True:
    conn, addr = sock.accept()
    try:
      # Use a byte buffer to handle incoming data
      data = b""
      # Read the request message (JSON) looking for the \n\n delimiter
      # We read slightly more than BUFFER_SIZE to catch the delimiter/preamble data
      while b"\n\n" not in data and len(data) < BUFFER_SIZE * 3: 
        chunk = conn.recv(BUFFER_SIZE)
        if not chunk:
          break
        data += chunk

      if not data:
        continue

      # Split JSON message from potential file data following it
      try:
        # Separate the JSON header from any subsequent binary data
        msg_json, file_data_preamble = data.split(b"\n\n", 1)
        msg = json.loads(msg_json.decode("utf-8"))
      except ValueError:
        # No delimiter found (e.g., standard LIST1 request)
        msg = json.loads(data.decode("utf-8").strip())
        file_data_preamble = b""
      except json.JSONDecodeError:
        print("[ERROR] Received non-JSON or malformed message.")
        continue

      # Handle the message. SEND_REQ handles the whole connection; LIST1 sends a simple response.
      resp = _handle_message(session, msg, conn, file_data_preamble)
      
      # For LIST1, send the JSON response
      if resp.get("type") == "LIST2":
        conn.sendall(json.dumps(resp).encode("utf-8"))

    except Exception as e:
      # print(f"[LISTENER ERROR] {e}") # Suppress for clean output
      pass
    finally:
      # If _handle_secure_send_req was called, it manages the socket life cycle, 
      # but we ensure closure here just in case of an earlier failure.
      conn.close()

# --- Message Handlers ---

def _handle_message(session: dict, msg: dict, conn: socket.socket, data: bytes) -> dict:
  msg_type = msg.get("type")
  
  if msg_type == "LIST1":
    return _handle_list1(session, msg)
  
  elif msg_type == "SECURE_SEND_REQ":
    # The SECURE_SEND_REQ handler manages the entire file transfer session
    _handle_secure_send_req(session, msg, conn, data)
    return {} # Return empty dict to prevent LIST1-style response

  return {"type": "ERROR"}

def _handle_list1(session: dict, msg: dict) -> dict:
  """Handles the LIST1 request (Milestone 4 mutual authentication)."""
  me_email = session["email"]
  me_id = id_for_email(me_email)

  from_id = msg.get("from_id", "")
  to_id = msg.get("to_id", "")
  from_pub = msg.get("from_pub", "")
  nonce = msg.get("nonce", "")
  sig = msg.get("sig", "")

  if to_id != me_id:
    return {"type": "LIST2", "accepted": False}

  # 1. Verify LIST1 Signature
  payload = f"LIST1|{from_id}|{to_id}|{nonce}".encode("utf-8")
  if not verify_sig(from_pub, payload, sig):
    return {"type": "LIST2", "accepted": False}

  my_contacts = get_my_contacts(session)

  # 2. Check if Sender is in Recipient's Contacts (Mutual Check)
  sender_email = None
  for email in my_contacts.keys():
    if id_for_email(email) == from_id:
      sender_email = email
      break

  if not sender_email:
    return {"type": "LIST2", "accepted": False}

  # 3. Check for Public Key Pinning/Integrity
  pinned = my_contacts.get(sender_email, {}).get("public_key", "")
  if pinned and pinned != from_pub:
    print(f"[ALERT] Public key mismatch for {sender_email}. Possible MITM.")
    return {"type": "LIST2", "accepted": False}

  if not pinned:
    # Pin the key for the first time
    upsert_contact_public_key(session, sender_email, from_pub)

  # 4. Generate LIST2 Response (Mutual Authentication/Key Exchange)
  my_nonce = nonce
  peer_nonce = "srv_" + nonce

  payload2 = f"LIST2|{to_id}|{from_id}|{my_nonce}|{peer_nonce}".encode("utf-8")
  sig2 = sign_bytes(session["private_key"], payload2)

  return {
    "type": "LIST2",
    "accepted": True,
    "name": session["name"],
    "from_pub": session["public_key"],
    "nonce": peer_nonce,
    "sig": sig2,
    "email": me_email
  }


def _handle_secure_send_req(session: dict, req_msg: dict, conn: socket.socket, data: bytes):
  """Handles the SECURE_SEND_REQ and manages the file transfer (Milestone 5)."""
  import os # Needed for sequence number in ACK

  me_email = session["email"]
  me_id = id_for_email(me_email)

  # Extract request data
  from_id = req_msg.get("from_id", "")
  to_id = req_msg.get("to_id", "")
  from_pub = req_msg.get("from_pub", "")
  seq_num = req_msg.get("sequence_num", "")
  file_name = req_msg.get("file_name", "received_file")
  file_size = req_msg.get("file_size", 0)
  original_hash = req_msg.get("original_hash", "")
  wrapped_key = req_msg.get("wrapped_key", "")
  aes_nonce_b64 = req_msg.get("nonce", "")
  aes_tag_b64 = req_msg.get("tag", "")
  req_sig = req_msg.get("sig", "")
  
  if to_id != me_id or not all([from_pub, seq_num, wrapped_key, req_sig, original_hash]):
    conn.sendall(json.dumps({"type": "SEND_ACK", "accepted": False, "reason": "Invalid Request or Missing Data"}).encode("utf-8"))
    return

  # 1. Mutual Contact/Authentication Check (Scenario 9)
  my_contacts = get_my_contacts(session)
  sender_email = None
  sender_name = "Unknown Contact"
  for email, info in my_contacts.items():
    # Check if sender is in contacts AND if their public key is the pinned one
    if id_for_email(email) == from_id and info.get("public_key") == from_pub:
      sender_email = email
      sender_name = info.get("name", sender_email)
      break

  if not sender_email:
    conn.sendall(json.dumps({"type": "SEND_ACK", "accepted": False, "reason": "Sender is not a recognized mutual contact."}).encode("utf-8"))
    return
    
  # 2. Verify Sender's Request Signature (Non-repudiation)
  req_payload = f"SEND_REQ|{from_id}|{to_id}|{seq_num}|{original_hash}".encode("utf-8")
  if not verify_sig(from_pub, req_payload, req_sig):
    conn.sendall(json.dumps({"type": "SEND_ACK", "accepted": False, "reason": "Sender signature verification failed."}).encode("utf-8"))
    return

  # 3. User Approval (FIXED: simplified input to minimize shell conflict)
  print(f"Contact '{sender_name} <{sender_email}>'  is sending a file. Hit enter.")
  
  # IMPORTANT: This input() will momentarily interrupt the shell prompt in this terminal.
  ans = input("> Accept (y/n)? ").strip().lower() 
  
  if ans != "y":
    conn.sendall(json.dumps({"type": "SEND_ACK", "accepted": False, "reason": "Transfer rejected by user."}).encode("utf-8"))
    return

  # 4. Send ACK Response (Anti-Replay)
  ack_seq_num = os.urandom(16).hex()
  ack_payload = f"SEND_ACK|{to_id}|{from_id}|{seq_num}|{ack_seq_num}|{original_hash}".encode("utf-8")
  ack_sig = sign_bytes(session["private_key"], ack_payload)
  
  ack_msg = {
    "type": "SEND_ACK",
    "accepted": True,
    "from_pub": session["public_key"],
    "sequence_num": ack_seq_num,
    "sig": ack_sig
  }
  
  conn.sendall(json.dumps(ack_msg).encode("utf-8"))
  
  # 5. Receive File Chunks (FIXED: Robustly read until file size is met)
  try:
    print(f"[RECEIVING] Expecting {file_size} bytes for {file_name}...")
    encrypted_data_chunks = [data] # Start with any preamble data
    bytes_recd = len(data)
    
    # Read the rest of the chunks until the expected file size is met
    conn.settimeout(10.0) # Set a timeout for file reception
    while bytes_recd < file_size:
      chunk = conn.recv(BUFFER_SIZE)
      if not chunk:
        # Client closed the connection unexpectedly before full transfer
        raise Exception("Client closed connection prematurely.")
      encrypted_data_chunks.append(chunk)
      bytes_recd += len(chunk)

    encrypted_file_bytes = b"".join(encrypted_data_chunks)[:file_size] # Take exactly the file size
    bytes_recd = len(encrypted_file_bytes)
    
    if bytes_recd != file_size:
      raise Exception(f"File size mismatch: Expected {file_size}, got {bytes_recd} bytes.")
      
  except Exception as e:
    print(f"[ERROR] Receiving file failed: {e}")
    conn.sendall(json.dumps({"type": "SEND_DONE", "success": False, "reason": "Network error during transfer."}).encode("utf-8"))
    return

  # 6. Decrypt and Integrity Check (Scenario 10)
  try:
    # Unwrap AES Key (RSA-OAEP Decryption)
    aes_key = unwrap_aes_key(wrapped_key, session["private_key"])
    if not aes_key:
      raise Exception("Failed to unwrap AES key.")
      
    # Prepare decryption data structure
    decrypt_data = {
      "ciphertext": _b64e(encrypted_file_bytes),
      "nonce": aes_nonce_b64,
      "tag": aes_tag_b64
    }
    
    # Decrypt file (AES-GCM verifies the tag implicitly)
    plaintext_bytes = decrypt_file(decrypt_data, aes_key)
    if plaintext_bytes is None:
      raise Exception("AES-GCM Tag verification failed (Integrity violation).")
      
    # Verify SHA256 Hash of Decrypted Content (Milestone 5 Requirement)
    calculated_hash = calculate_sha256(plaintext_bytes)
    if calculated_hash != original_hash:
      raise Exception(f"Decrypted file hash mismatch: Expected {original_hash}, calculated {calculated_hash}.")

    # 7. Save File
    output_path = Path("received") / file_name
    output_path.parent.mkdir(exist_ok=True)
    output_path.write_bytes(plaintext_bytes)
    
    print(f"[SUCCESS] File saved as: {output_path.resolve()}")
    
    # 8. Send DONE Confirmation
    conn.sendall(json.dumps({"type": "SEND_DONE", "success": True}).encode("utf-8"))

  except Exception as e:
    print(f"[ERROR] File processing/verification failed: {e}")
    conn.sendall(json.dumps({"type": "SEND_DONE", "success": False, "reason": f"Integrity/Decryption Failed: {e}"}).encode("utf-8"))